...
def UsingPySubPlexed():
    ...
    PySubPlexed is perfect for spawning n subprocesses for example, handling them and waiting for results and sorting
    results and keeping all the results in order while waiting for results to come back in. All in one line inside
    a program that leverages PySubPlexed in this way.
    ...
    Example application:
    For example 2 default pings in half the time or 8 in the time of 1 is how one should
    think about applying PySubPlexed. Because all the handling is done and in one line
    everything comes back into one variable in order and tagged if specified.
